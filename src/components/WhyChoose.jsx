  ...
  return (
    <section id="why" className="py-16">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {items.map(i=>(
          <div key={i.title} className="p-6 rounded-2xl card-glass flex gap-4 items-start">
            <div className="text-antique-gold mt-1"><FiCheckCircle size={22} /></div>
            <div>
              <h4 className="font-semibold">{i.title}</h4>
              <p className="muted text-sm mt-1">{i.text}</p>
            </div>
          </div>
        ))}
      </div>
